# Telegram Bot Analytics - Full Project Guide

Полнофункциональный Telegram бот для сбора статистики сообщений, анализа пользователей с помощью Gemini AI и веб-интерфейса для анализа.

## 🚀 Быстрый Старт

### Требования

- Docker & Docker Compose
- Node.js 18+ (для локальной разработки)
- Telegram Bot Token (от [@BotFather](https://t.me/botfather))
- Gemini API Key (от [Google AI Studio](https://aistudio.google.com))

### Установка

1. **Клонируйте репозиторий**

   ```bash
   git clone <repo>
   cd bot_telegram
   ```

2. **Скопируйте .env файл**

   ```bash
   cp .env.example .env
   ```

3. **Обновите переменные окружения в `.env`**

   ```env
   TELEGRAM_TOKEN=ваш_токен_здесь
   GEMINI_API_KEY=ваш_gemini_ключ_здесь
   DB_PASSWORD=вашпароль
   ```

4. **Запустите проект через Docker**

   ```bash
   docker-compose up --build
   ```

   Проект будет доступен:

   - 🤖 Бот: подключен к Telegram
   - 🌐 Веб-интерфейс: http://localhost:3000
   - 📊 API: http://localhost:3001
   - 🗄️ PostgreSQL: localhost:5432
   - 🔴 Redis: localhost:6379

### Локальная разработка (без Docker)

```bash
# Установка зависимостей
cd bot && npm install
cd ../web && npm install

# Запуск бота
npm run dev

# Запуск API (в отдельном терминале)
npm run dev:api

# Запуск веб-интерфейса (в отдельном терминале)
cd web && npm run dev
```

---

## 🎯 Функционал

### 1. **Сбор данных**

- Автоматическое сохранение всех текстовых сообщений в группе
- Отслеживание информации о пользователях (username, имя, фамилия)

### 2. **Статистика чата** (`/stats`)

- Топ-10 пользователей по количеству сообщений
- Фильтрация по времени: сегодня, неделя, месяц, всё время
- Кэширование результатов в Redis (20 минут по умолчанию)

Команда:

```
/stats
```

Навигация через inline кнопки:

- 📈 Today
- 📊 Week
- 📅 Month
- 🔄 All Time

### 3. **Анализ пользователя** (`/analyze`)

Использует Gemini API для анализа стиля общения пользователя.

Команды:

```
/analyze @username          # Анализ по username
(reply to message) /analyze # Анализ автора сообщения
```

Результат анализа включает:

- 💬 Стиль общения (формальный/неформальный)
- 📌 Основные темы
- ⏰ Паттерны активности
- 😊 Тональность (позитивная/нейтральная/негативная)
- ✨ Особенности (использование эмодзи, частые вопросы)

### 4. **Веб-интерфейс**

Простая страница для анализа пользователя через браузер.

- URL: http://localhost:3000
- Введите username → получите анализ
- Интеграция с API для получения данных

---

## 📁 Архитектура Проекта

```
bot_telegram/
├── bot/                    # Node.js бэкенд
│   ├── src/
│   │   ├── index.ts       # Entry point бота
│   │   ├── api.ts         # REST API сервер
│   │   ├── database/      # Работа с PostgreSQL
│   │   ├── models/        # Классы для работы с данными
│   │   │   ├── UserModel.ts
│   │   │   ├── ChatModel.ts
│   │   │   └── MessageModel.ts
│   │   ├── services/      # Бизнес-логика
│   │   │   ├── StatsService.ts
│   │   │   ├── GeminiService.ts
│   │   │   └── RedisService.ts
│   │   ├── handlers/      # Обработчики команд бота
│   │   │   ├── messageHandler.ts
│   │   │   ├── statsHandler.ts
│   │   │   ├── analyzeHandler.ts
│   │   ├── types/         # TypeScript интерфейсы
│   │   └── utils/         # Утилиты
│   ├── src/__tests__/     # Unit тесты (Vitest)
│   ├── package.json
│   ├── tsconfig.json
│   └── Dockerfile
│
├── web/                    # Next.js фронтенд
│   ├── app/
│   │   ├── page.tsx       # Главная страница
│   │   ├── layout.tsx     # Layout
│   │   ├── globals.css    # Стили
│   │   ├── components/    # React компоненты
│   │   └── types/         # TypeScript интерфейсы
│   ├── package.json
│   ├── tsconfig.json
│   ├── next.config.js
│   └── Dockerfile
│
├── docker-compose.yml      # Оркестрация сервисов
├── .env                    # Переменные окружения
├── .env.example
└── README.md
```

### Модели Данных

#### Users

```sql
id (PRIMARY KEY)
telegram_id (UNIQUE)
username
first_name
last_name
created_at
```

#### Chats

```sql
id (PRIMARY KEY)
telegram_id (UNIQUE)
title
created_at
```

#### Messages

```sql
id (PRIMARY KEY)
chat_id (FOREIGN KEY → chats)
user_id (FOREIGN KEY → users)
text
created_at
```
